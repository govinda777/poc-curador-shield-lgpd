...
async function verifyPOC() {
    ...
    const aclNode = await AccessService.wrapKeyForDoctor(PATIENT_WALLET, record.record_id, docPubKey, PATIENT_SIGNATURE // Using the signature as the "Master Key" to be wrapped
    );
    ...
    if (!node)
        throw new Error("ACL Node not found");
